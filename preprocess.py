def preprocess_w_lat_lons_time(netcdf_file_path):
    ...
    timestamps = f.variables['pvt_timestamp_utc'][:]
    ...
    filtered_timestamps = np.array([timestamps[i] for i, j in to_keep_indices])
    ...
    assert np.array(fit_data).shape[0] == len(label_data) == np.array(specular_point_lats).shape[0] == np.array(specular_point_lons).shape[0] == filtered_timestamps.shape[0], \
    ...
    return fit_data, label_data, specular_point_lats, specular_point_lons, filtered_timestamps, time_coverage_start, time_coverage_end